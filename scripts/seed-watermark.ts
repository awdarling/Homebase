...
async function main() {
  ...
  const torInserted = must('time_off_requests', await supabase.from('time_off_requests').insert([
    { company_id: cid, employee_id: E.jordan, start_date: isoDate(nextWeekMon), end_date: isoDate(addDays(nextWeekMon, 2)), reason: 'Family vacation', status: 'pending', requested_at: now.toISOString(), decided_at: null, decided_by: null },
    { company_id: cid, employee_id: E.aisha,  start_date: isoDate(twoWeeks),    end_date: isoDate(twoWeeks),               reason: 'Medical appointment', status: 'approved', requested_at: now.toISOString(), decided_at: now.toISOString(), decided_by: null },
    { company_id: cid, employee_id: E.noah,   start_date: isoDate(threeWeeks),  end_date: isoDate(addDays(threeWeeks, 1)), reason: 'Out of town', status: 'approved', requested_at: now.toISOString(), decided_at: now.toISOString(), decided_by: null },
    { company_id: cid, employee_id: E.lily,   start_date: isoDate(nextWeekMon), end_date: isoDate(nextWeekMon),            reason: 'Personal day', status: 'denied', requested_at: now.toISOString(), decided_at: now.toISOString(), decided_by: null },
    { company_id: cid, employee_id: E.zoe,    start_date: isoDate(fourWeeks),   end_date: isoDate(addDays(fourWeeks, 3)),  reason: 'Summer trip', status: 'pending', requested_at: now.toISOString(), decided_at: null, decided_by: null },
    { company_id: cid, employee_id: E.connor, start_date: isoDate(twoWeeks),    end_date: isoDate(addDays(twoWeeks, 1)),   reason: 'Family event', status: 'approved', requested_at: now.toISOString(), decided_at: now.toISOString(), decided_by: null },
  ]).select())
