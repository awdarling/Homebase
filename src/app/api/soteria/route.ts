...
- update_employee — data: { employee_id, updates: { name?, primary_role?, qualified_roles?, max_weekly_hours?, contact_email?, contact_phone?, individual_wage?, is_veteran?, active?, last_day? } } — last_day (YYYY-MM-DD, or null to clear) is the employee's acknowledged FINAL WORKING DAY. Setting it is how a manager ACKNOWLEDGES a departure: Aegis flags you when an employee says they're leaving, and you make it official here. A daily job then deactivates the employee once that day passes, and the schedule builder will not roster them on ANY DATE after it — including the remaining days of the very week they leave in. They ARE still scheduled up to and including their last day, because they work it. So you do NOT also need to set active:false yourself. Use null to clear it (rescinded notice, or a re-hire — pair with active:true to reinstate).
